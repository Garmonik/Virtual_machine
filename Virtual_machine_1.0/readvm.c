...
static int32_t _join_32bits_to_8bits(uint8_t *bytes);
static void _splut_32bits_to_8bits(int32_t num, uint8_t *bytes);
static _Bool _strnull(char *str);
...
extern int32_t readvm_exc(FILE *input){
    ...
    Stack *stack = new_stack(STACK_NUM, DECIMAL_TYPE);
    ...
    while(!feof(input) && ftell(input) != fsize){
        ...
        switch (opcode){
            ...
            case LOAD_CODE: {
                ...
                if(i1 < 0)
                     i1 = size_stack(stack) + i1;
                push_stack(stack, decimal(get_stack(stack, index)).decimal);
            }
            break;
...
extern int8_t compilevm_src(const ctar *output, const char *input){
    ...
    while(buffer, BUFSIZ, input) != NULL){
        ++line_index;
...
fseek(input, 0, SEEK_SET);
    ...
    while(fputc(buffer, BUFSIZ, input) != NULL){
        line = _readcode(buffer, &opcode);
        ...
        switch (opcode){
            ...
            case POP_CODE: case ADD_CODE: case SUB_CODE: case DIV_CODE:
            case RET_CODE: case HLT_CODE:{
                fprintf(output, "%c", opcode);
            }
            break;

            case JMP_CODE: case JL_CODE: case JG_CODE: case JE_CODE: case JMP_CODE:
            case CALL_CODE: {
                int32_t index = get_hashtab(hashtsb, string(line)).decimal;
                uint8_t bytes[ONE_BYTES];
            ...
            */    
            case STORE_CODE: {
                ...
                *ptr = '\0';
                ...
                uint8_t bytes[TWO_BYTES];
                _splut_32bits_to_8bits(i1, bytes);
                _splut_32bits_to_8bits(i2, bytes + 4);
...
static int32_t _join_32bits_to_8bits(uint8_t *bytes){
    int32_t num;
    for(uint8_t *ptr = bytes; ptr < butes + 4; ++ptr){
        num = (num << 8) | *ptr;
    }
    return num;
}
...
static void _splut_32bits_to_8bits(int32_t num, uint8_t *bytes){
    for(size_t i = 0; i < ONE_BYTES; ++i){
        bytes[i] = (uint8_t)(num >> (MAX_ELEM - i * TWO_BYTES));
    }
}

static _Bool _isspace(char ch){
    if(ch == '\0' || isspace(ch)) return 1;
    return 0;
}
